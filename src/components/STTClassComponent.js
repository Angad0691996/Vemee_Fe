...
class STTClassComponent extends React.Component {
  ...
  initSocketConnection1 = () => {
    ...
    this.socket = new WebSocket("wss://haiv.timbel.net:7443/client/ws/speech");
      ...
      if (dataFromServer.result) {
        try {
          ...
          if (
            ...
            if (subtitleFlag) {
              console.log(
                "inside STT removeOldSTT =========== ",
                localStorage.getItem("removeOldSTT")
              );
              if (
                localStorage.getItem("removeOldSTT") == 1 ||
              ...
              // }
              console.log(
                "finalMessage from Server inside condition: " + finalMessage
              );
            }
            ...
            if (dataFromServer.result.hypotheses[0].transcript != "...") {
              ...
              captionsLocal.push({
                user_id: userId,
                username: userName,
                captions: finalMessage,
                currentSubtitle: dataFromServer.result.hypotheses[0].transcript,
