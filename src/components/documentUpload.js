...
import file_upload from "../images/documents.svg";
...
function DocumentUpload(props) {
  ...
  const uplaodDocument = async (e) => {
    ...
    try {
      ...
      const { data } = await Axios.post(
        `${ENDPOINTURL}${apiRoutes.document_upload}`,formData,
        {
          headers: {
      ...
      let uploadeddocumentcount = data.length;
      if (data.status === 200) {
        setComponentStatus({
          status: "OK",
  ...
  const getDocumentList = async () => {
    ...
    try {
      const data = await Axios.get(
        `${ENDPOINTURL}${apiRoutes.document_get}?meetingId=${currentMeetingId}&source=Documents`,
  ...
  const deleteDocument = async (documentId) => {
    try {
      ...
      setComponentStatus({
        status: "error",
        message: constant.document_del_wrong,
      });
    }
  };
    ...
    <>
      {
        <Modal
          isOpen={modal}
          toggle={toggle}
          style={{ maxWidth: "600px", borderRadius: "10px", padding: "0" }}
          className={"className"}
        >
          <div class="upload_title_box">
            <ul class="d-flex align-items-center ">
              <li>
                <a
                  href="#why1"
                  onClick={() => setModelContentHandler("upload")}
                  class="upload_tabs active"
                >
                  Add New Documents
                </a>
              </li>
                      ...
                      documentList.map((doc, i) => (
                        <div class="upload_list_box" id="UploadDocment" key={i}>
                          <div class="upload_list_box_inr">
                            <a href={doc.document_url} target="blank">
                              {doc.document_title}
                            </a>
      ...
      {localStorage.getItem("meeting_role") === "Attendee" && (
        <Modal
          isOpen={modal}
          toggle={toggle}
          style={{ maxWidth: "600px", borderRadius: "10px", padding: "0" }}
          ...
          <div class="upload_title_box">
            <ul class="d-flex align-items-center ">
              <li>
                <a
                  href="#why2"
                  onClick={() => setModelContentHandler("list")}
                  class="upload_tabs"
                >
